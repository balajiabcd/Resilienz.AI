...
def generate_suppliers() -> pd.DataFrame:
    ...
    for i, (name, city, country) in enumerate(german_companies):
        ...
        # Assign risk categories: 2 HIGH, 3 MEDIUM, rest LOW
        if i == 0 or i == 1:
            risk_cat = "HIGH"
            reliability = round(random.uniform(0.70, 0.79), 2)
            avg_delay = round(random.uniform(2.0, 5.0), 1)
        elif i in [2, 3, 4]:
            risk_cat = "MEDIUM"
            reliability = round(random.uniform(0.80, 0.89), 2)
            avg_delay = round(random.uniform(1.0, 2.5), 1)
        else:
            risk_cat = "LOW"
            reliability = round(random.uniform(0.90, 0.99), 2)
            avg_delay = round(random.uniform(0.0, 1.0), 1)
        ...
        orders.append(
            {
                ...
                "supplier_name": suppliers_df.loc[
                    suppliers_df["supplier_id"] == supplier_id, "supplier_name"
                ].values[0],
                "part_number": part[0],
...
# ==============================================================================
def generate_supplier_status(
    orders_df: pd.DataFrame, suppliers_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    delay_reasons = [
        ...
        "Suez Canal disruption - Red Sea route closed",
        "Cape of Good Hope rerouting - 14 day delay",
    ]

    asian_countries = ["JP", "CN", "KR", "IN", "TW", "SG", "MY", "TH", "VN"]

    statuses = []
    for _, order in orders_df.iterrows():
        supplier = suppliers_df[
            suppliers_df["supplier_id"] == order["supplier_id"]
        ].iloc[0]

        # Check if supplier is from Asia - affected by Suez Canal disruption
        is_asian = supplier["country"] in asian_countries

        # Lower reliability suppliers are more likely to be delayed
        delay_probability = 1 - supplier["reliability_score"]

        if is_asian:
            # Asian suppliers get forced delays due to Suez Canal/South Africa route
            delay_days = random.randint(10, 16)  # 10-16 days delay
            ...
            is_delayed = random.random() < (delay_probability + 0.15)
            if is_delayed:
...
def generate_inventory(orders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, part in enumerate(parts.iterrows()):
        part_data = part[1]
        daily_consumption = round(random.uniform(5.0, 50.0), 1)

        # Ensure most products have adequate stock (above min)
        # Only make 2-3 products critical for demo purposes
        if idx < 2:
            # 2 critical products - below min stock
            min_stock = int(daily_consumption * random.randint(4, 7))
            stock_qty = int(min_stock * random.uniform(0.3, 0.6))
        else:
            # Rest have adequate stock
            min_stock = int(daily_consumption * random.randint(3, 5))
            stock_qty = int(min_stock * random.uniform(1.2, 2.5))
...
def load_into_vector_db(events_df: pd.DataFrame):
    ...
    """
    print("Loading events into ChromaDB (Vector DB)...")
    ...
    try:
        ...
        collection = client.create_collection(
            name="global_events",
            metadata={
                "description": "Global supply chain disruption events for Resilienz.AI"
            },
        )
        ...
        collection.add(
            ...
            metadatas=events_df.drop(columns=["description"]).to_dict(
                "records"
            ),  # Extra info
        )

        print(
            f"   {len(events_df)} events loaded into ChromaDB at: {config.VECTOR_STORE_DIR}"
        )
...
if __name__ == "__main__":
    ...
    print(f"    [dir] vector_store/ (ChromaDB)")
